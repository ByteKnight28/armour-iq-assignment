...
export async function storeSecret({ name, value, description = "" }) {
...
export async function retrieveSecret({ name }) {
    const store = await loadStore();
...
export async function rotateSecret({ name, new_value }) {
    const store = await loadStore();
    const secret = store.secrets[name];
    if (!secret) return { success: false, error: `Secret '${name}' not found.` };
...
export async function auditAccess({ name }) {
    const store = await loadStore();
...
export async function revokeSecret({ name }) {
    const store = await loadStore();
